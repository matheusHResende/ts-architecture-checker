export class Module{
    ...
    clean(): void {
        ...
        this.allowed = [...new Set(this.allowed)].filter(file => file.startsWith("/") && !file.endsWith("*"))
        this.forbidden = [...new Set(this.forbidden)].filter(file => file.startsWith("/") && !file.endsWith("*"))
        this.required = [...new Set(this.required)].filter(file => file.startsWith("/") && !file.endsWith("*"))
    }
}
